...
namespace UnSealer.Core.Utils.Dnlib.CawkRuntime.ConversionBack
{
    public class ConvertBack
    {
        ...
        public static DynamicMethod ModRunner(string resourceName, int xorKey, int methodIndex, object[] parameters, MethodBase Meth, System.Reflection.Assembly assembly)
        {

            DynamicMethod value;
            if (cache.TryGetValue(methodIndex, out value))//Check cache to see if method has already been created
            {
                return (DynamicMethod)value.Invoke(null, parameters);//if it has directly invoke the method instead of converting it to dynamicmethod
            }
            else
            {
                MethodBase callingMethod = Meth;//get calling method, using this can prevent invocation and dynamic unpacking
                byte[] decryptedStream = ReadResouce(resourceName, xorKey, assembly);
                return ConversionModdedBack(decryptedStream, methodIndex, parameters, callingMethod);//if not convert the method to a dynamic method

            }
        }

        private static byte[] ReadResouce(string resourseName, int xorKeyValue, System.Reflection.Assembly assembly)
        {
            Stream manifestResourceStream = assembly.GetManifestResourceStream(resourseName);
            byte[] array = new byte[manifestResourceStream.Length];
            manifestResourceStream.Read(array, 0, array.Length);
           
            return DecryptXORByteArray(array, (byte)xorKeyValue);
        }
        ...
        public static DynamicMethod ConversionBack(byte[] bytes, int ID, object[] parameters, MethodBase callingMethod)
        {
            ...
            for (var i = 0; i < instructionCount; i++)
            {
                ...
                var opcode = binaryReader.ReadInt32();//read opcode short this will relate to the correct opcode
                OpCode opc;
        ...
        public static DynamicMethod ConversionModdedBack(byte[] bytes, int ID, object[] parameters, MethodBase callingMethod)
        {
            MethodBody methodBody = callingMethod.GetMethodBody();//get calling methods body 
            BinaryReader binaryReader = new BinaryReader(new MemoryStream(bytes));//cast byte[] Position
            ...
            foreach (var localVariableInfo in locs)
                allLocals.Add(ilGenerator.DeclareLocal(localVariableInfo.LocalType));//declare the local for use of stloc,ldloc/ldloca
            var exceptionHandlersCount = binaryReader.ReadInt32();//read amount of exception handlers
            processExceptionHandlerMod(binaryReader, exceptionHandlersCount, callingMethod, _allExceptionHandlerses);//convert exception handlers
            var sortedExceptionHandlers = fixAndSortExceptionHandlers(_allExceptionHandlerses);//we need to sort the exception handlers incase there is multiple handlers that start at the same instruction
            var instructionCount = binaryReader.ReadInt32();//read the amount of instructions
            var _allLabelsDictionary = new Dictionary<int, Label>();
            ...
            for (var i = 0; i < instructionCount; i++)
            {
                ...
                HandleModdedOpType(operandType, opc, ilGenerator, binaryReader, _allLabelsDictionary, allLocals);//we process the instruction with ilgenerator
            }
            lock (locker)//we lock threads here to prevent exceptions of item already exists
            {
                if (!cache.ContainsKey(ID))
        ...
        private static void HandleModdedOpType(int opType, OpCode opcode, ILGenerator ilGenerator, BinaryReader binaryReader, Dictionary<int, Label> _allLabelsDictionary, List<LocalBuilder> allLocals)
        {
            switch (opType)
            {
                case 226:
                ...
                case 193:
                    ...
                    break;
                case 75:
                    InlineStringEmitterMod(ilGenerator, opcode, binaryReader);
                    break;
                case 245:
                    InlineIEmitter(ilGenerator, opcode, binaryReader);
                    break;
        ...
        private static void InlineStringEmitterMod(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader)
        {

            int count = binaryReader.ReadInt32();
            byte[] array = binaryReader.ReadBytes(count);
            array = DecryptXORByteArray(array, binaryReader.ReadByte());
        ...
        public static void processExceptionHandlerMod(BinaryReader bin, int count, MethodBase method, List<ExceptionHandlerClass> _allExceptionHandlerses)
        {
            for (var i = 0; i < count; i++)
            {
                //catchType
        ...
        private static byte[] DecryptXORByteArray(byte[] resourseArray, byte xorKeyValueByte)
        {
            for (int i = 0; i < resourseArray.Length; i++)
            {
                int num = i;
                resourseArray[num] ^= xorKeyValueByte;
            }
